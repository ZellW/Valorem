# Replicate Eric Chiang's Python code of churn model published at 
...
#bad data so need to clean it up
colnames(data)[21] <- "churn"

# dependent variable
y <- if_else(data$churn == "False.", FALSE, TRUE)

X <- select(data, -State, -Area.Code, -Phone, -churn)
X$Int.l.Plan <- if_else(data$Int.l.Plan == "no", FALSE, TRUE)
X$VMail.Plan <- if_else(data$VMail.Plan == "no", FALSE, TRUE)
X.scaled <- scale(X)


# compare 3 classifiers with k-fold cross-validation
compareClassifiers <- function(k=5) {
  ...
  # add the k tables together for full cross-validated confusion matrices
  makeCM <- function(x) {
    out <- Reduce('+',x)
...
myClassifiers <- compareClassifiers()

# OK, so now I have predicted probabilities for everybody.  
# split data into groups corresponding to probability ranges.
# This function takes a probability matrix (pm) as stored in
# the third element of the list that compareClassifiers() returns.
...
svmsum <- summarizeThese(groupThese(myClassifiers[[3]][['SVM']]))
rfsum  <- summarizeThese(groupThese(myClassifiers[[3]][['rF']]))
knnsum <- summarizeThese(groupThese(myClassifiers[[3]][['KNN']]))

## @knitr accuracy
# Cross-validated accuracy rates
round(colMeans(myClassifiers[[1]]),3)
