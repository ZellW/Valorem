...
### Manage Large File Size

The file size for for transactions.csv is 1.7GB.  Let's break this into smaller individual files.
...
train <- fread(paste0(PATH,"train.csv"), stringsAsFactors = TRUE, select="msno")
test <- fread(paste0(PATH,"sample_submission_zero.csv"), stringsAsFactors = TRUE, select="msno")
...
for (i in 1:(num_outputs-49)) { # Should be 1:num_outputs -- Changed for successful Kaggle Kernel run
  cat("Round",i,"-- Uploading user_logs\n")
  if (i==1) {
    users <- fread(paste0(PATH,"user_logs.csv"), sep=",", na.strings = "", stringsAsFactors=T, nrow=nrow_size)
  }
  else if (i==num_outputs) {
    users <- fread(paste0(PATH,"user_logs.csv"), stringsAsFactors = TRUE, skip=((i-1)*nrow_size+1), col.names = user_col_names)
  }
  else {
    users <- fread(paste0(PATH,"user_logs.csv"), stringsAsFactors = TRUE, nrow=nrow_size, skip=((i-1)*nrow_size+1), col.names = user_col_names)
  }
  ...
  write.table(users, paste0("user_logs_subset_",i), sep=",", dec=".", quote=FALSE, row.names=FALSE)
  rm(users); gc()
}
